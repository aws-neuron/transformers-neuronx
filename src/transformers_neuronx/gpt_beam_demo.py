...
def demo(model_name, model_cls, amp_callback):
    ...
    run_parser.add_argument('--batch_size', type=int, default=6, help="Input batch size")
...
def run(args, model_name, model_cls):
    ...
    with torch.inference_mode():
        ...
        generated_sequence = model.beam_search(input_ids, beam_scorer, sequence_length=model.config.n_positions, logits_processor=logits_processor, config=model.config)
