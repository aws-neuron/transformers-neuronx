...
from transformers_neuronx.decoder import DecoderLayer, DecoderParameterBuilder


class DecoderLmHeadTopKForSamplingNoEmbedding(torch.nn.Module):
    ...
    def forward(self, *inputs):
        ...
        return self.program.topk_device_to_host()
    ...
    def _build_program(self):
        if self.unroll == self.num_layers:
            ...
            program = DecoderTopKProgramFullyUnrolled(hlo_modules, num_inputs, self.tp_degree)
        else:
            ...
            num_inputs = len(self.inputs_sdim)
            program = DecoderTopKProgramMultiLayer(hlo_modules, ln_lm_head_hlo_module, num_inputs,
    ...
    def _hlo_fully_unrolled(self, n_positions):
        ...
        def fully_unrolled(scribe):
            ...
            topk = self.ln_lm_head_builder(hidden, ln_f_weight, ln_f_bias, head_weight, head_bias)
            topk_values_sizes = tuple(topk.shape_proto.tuple_shapes[0].dimensions)
            topk_indices_sizes = tuple(topk.shape_proto.tuple_shapes[1].dimensions)
            topk_values = dtype[topk_values_sizes].GetTupleElement(topk, tuple_index=0)
            topk_indices = dtype[topk_indices_sizes].GetTupleElement(topk, tuple_index=1)
            outputs = [topk_values, topk_indices, *out_caches]
...
class DecoderTopKProgram:
    ...
    def setup(self, layers, ln_lm_head_params):
        ...
        self.topk_values_buffer = self.manipulator.duplicate(self.topk_values_buffer)
        self.topk_indices_buffer = self.manipulator.duplicate(self.topk_indices_buffer)
    ...
    def run(self, bucket_id):
        raise NotImplementedError(DecoderTopKProgram)

    def topk_device_to_host(self):
        values = self.manipulator.unshard_along(self.topk_values_buffer, dim=0)
        indices = self.manipulator.unshard_along(self.topk_indices_buffer, dim=0)
...
class DecoderTopKProgramFullyUnrolled(DecoderTopKProgram):

    def __init__(self, hlo_modules, num_inputs, tp_degree):
        super().__init__(hlo_modules, num_inputs, tp_degree)
        first_hlo, *_ = hlo_modules
        self.topk_values_buffer = compiler.gen_zero_output(first_hlo, 0)
        self.topk_indices_buffer = compiler.gen_zero_output(first_hlo, 1)
    ...
    def setup(self, layers, ln_lm_head_params):
        ...
        for npos, memory in zip(self.n_positions_list, self.memories):
            ...
            output_tensors = [self.topk_values_buffer, self.topk_indices_buffer]
...
class DecoderTopKProgramMultiLayer(DecoderTopKProgram):
    ...
    def __init__(self, hlo_modules, ln_lm_head_hlo_module, num_inputs, num_layers, unroll, tp_degree):
        ...
        self.topk_values_buffer = compiler.gen_zero_output(ln_lm_head_hlo_module, 0)
        self.topk_indices_buffer = compiler.gen_zero_output(ln_lm_head_hlo_module, 1)
        self.unroll = unroll
    ...
    def setup(self, layers, ln_lm_head_params):
        ...
        for memories, multi_layer in zip(self.multi_layers_memories, multi_layers):
            for npos, memory in zip(self.n_positions_list, memories):
                ...
                memory.setup(input_tensors, output_tensors)
        self.ln_lm_head_memory.setup([hidden_buffer, *ln_lm_head_params],
                                     [self.topk_values_buffer, self.topk_indices_buffer])
