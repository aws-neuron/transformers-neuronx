...
def build_gptneox_hlo_module(config, n_active_tokens, n_positions):
    gptneox = gen_scribable_gptneox(config, n_active_tokens, n_positions)
    return compiler.compile_py_func(gptneox)


def attention(hidden, q_weight, q_bias, k_weight, k_bias, v_weight, v_bias,
...
def gptneox(hidden, pos_embd, cache_offset, mask, blocks_caches, blocks_params, ln_lm_head_params, config):
    scribe = hidden.scribe
    dtype = hidden.dtype
    outputs = []
    for (key_cache, value_cache), block_params in zip(blocks_caches, blocks_params):
        ...
        hidden, out_key_cache, out_value_cache = block(hidden, ln_1_weight, ln_1_bias,
          attn_q_weight, attn_q_bias, attn_k_weight, attn_k_bias,
          attn_v_weight, attn_v_bias, attn_out_weight, attn_out_bias,
          pos_embd, key_cache, value_cache, cache_offset, mask, ln_2_weight, ln_2_bias,
          mlp_in_weight, mlp_in_bias, mlp_out_weight, mlp_out_bias,
...
def gen_scribable_gptneox(config, n_active_tokens, n_positions):
    ...
    def scribable(scribe):
        ...
        return gptneox(hidden, pos_embd, cache_offset, mask, blocks_caches, blocks_params, ln_lm_head_params, config)
