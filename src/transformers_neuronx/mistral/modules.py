...
class MistralForCausalLM(module.PretrainedModel):
...
class MistralModel(module.LowMemoryModule):
    ...
    def __init__(self, config):
        ...
        self.layers = module.LowMemoryModuleList([MistralDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = MistralRMSNorm(config)


class MistralRMSNorm(module.LowMemoryModule):
...
class MistralDecoderLayer(module.LowMemoryModule):

    def __init__(self, config):
        super().__init__()
        self.self_attn = MistralAttention(config)
        self.mlp = MistralMLP(config)
        self.input_layernorm = MistralRMSNorm(config)
        self.post_attention_layernorm = MistralRMSNorm(config)


class MistralAttention(module.LowMemoryModule):
...
class MistralMLP(module.LowMemoryModule):
