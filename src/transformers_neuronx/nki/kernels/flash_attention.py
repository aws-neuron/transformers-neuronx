...
from neuronxcc.nki.isa import sb_transpose, nc_matmul
from neuronxcc.starfish.penguin.targets.nki.tensor import optional_mask, local_tile

from transformers_neuronx.nki.compile import nki_call

def matmul(x: local_tile, y: local_tile, *, transpose_x=False, mask: optional_mask = None) -> local_tile:
  r"""
  ``x @ y`` matrix multiplication of ``x`` and ``y``.

  .. note::
      For optimal performance on hardware, use ``nki.isa.nc_matmul`` or call this function
      with ``transpose_x=True``. Also use nki.isa.nc_matmul to access low-level features
      of the TensorEngine.
  ...
  :param transpose_x: if true, ``x`` is transposed before matmul,
                      hence ``x``'s partition dimension must match
                      ``y``'s partition dimension.
  :param mask: executes normally iff mask is evaluated to true; otherwise, the op is nullified.

  :return: ``x @ y`` or ``x.T @ y`` if transpose_x=True
  """
  if not transpose_x:
    # sema.nki_assert(mask is None, "mask is not supported when transpose_x=False")
    x = sb_transpose(x, mask=mask)
  return nc_matmul(lhs=x, rhs=y, mask=mask)
...
def flash_fwd(q, k, v, o, lse):
  ...
  for i in nl.affine_range(n_tile_q):
    ...
    for k_i in nl.affine_range(num_k_tile_per_large_tile):
      ...
      qk_psum[i_q_p,i_q_f] += matmul(q_tile, k_tile, transpose_x=True) # (p(128), 512)
    ...
    for k_i in nl.affine_range(LARGE_TILE_SZ//B_P_SIZE):
        ...
        pv_psum[i_q_p,i_f_d] += matmul(p[i_q_p, k_i*B_P_SIZE + i_f_128],v_local[k_i, i_q_p, i_f_d], transpose_x=False) # (128, 128) (p(Br), d)
    ...
    for j in nl.sequential_range(num_large_k_tile-1): # range done not work for affine_range(1,n_tile_k)
      ...
      for k_i in nl.affine_range(num_k_tile_per_large_tile):
        ...
        qk_psum[i_q_p,i_q_f] += matmul(q_tile, k_tile, transpose_x=True, mask=multiplication_required_selection) # (p(128), 512)
