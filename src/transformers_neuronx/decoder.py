...
class DecoderLmHeadForSamplingNoEmbedding(torch.nn.Module):
    ...
    def new_layer(self):
        *_, n_positions = self.n_positions_list
        ...
        shard_over_batch = is_multi_query_attn and (self.batch_size >= self.tp_degree) and (self.batch_size % self.tp_degree == 0)
        layer = DecoderLayer(self.tp_degree, n_positions, self.batch_size, self.attention_head_size,
                             ...
                             n_head=self.n_head, n_kv_head=self.n_kv_head, shard_over_batch=shard_over_batch)
